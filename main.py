...
class CUTS(object):
    def __init__(self, args: CUTSopt.CUTSargs, log, device="cuda"):
        ...
        if self.args.data_pred.model == "multi_mlp":
            #输入：获取一段历史时间序列数据（input_step个时间步）
            #输出：预测未来pred_step步的值
        ...
        self.data_pred_loss = nn.MSELoss() # 均方误差MSE损失函数类，后续通过前面这个来计算损失函数
        ...
        #创建了一个Adam优化器实例，该优化器将用于训练 fitting_model 模型。
        #设置了学习率和权重衰减，使得优化器在训练过程中根据这些超参数调整模型参数。
        #优化模型的所有参数，即fitting_model.parameters()，这意味着模型的权重和偏置将通过优化器进行更新，以最小化损失函数。
        # 初始化因果图
    ...
    def latent_data_pred(self, x, y, observ_mask):
        ...
        bs, n, m, t, d = x.shape  # 获取输入数据维度
        self.fitting_model.train() # 设置模型为训练模式
...
#为什么会有两个损失函数？
#稀疏性损失：鼓励图的稀疏性，即图中的边越少越好。
#数据预测损失：鼓励预测值与真实值尽可能接近。
    ...
    def train(self, data, observ_mask, original_data, true_cm=None):
        ...
        auc = 0
        #  训练循环中的数据更新策略
        for epoch_i in range(self.args.total_epoch):#训练次数   
            if "every" in self.args.fill_policy:#每隔一段时间更新一次数据
                update_every = int(self.args.fill_policy.split("_")[-1])
                if (epoch_i+1) % update_every == 0:
                    data = data_pred # 使用预测数据更新
                    print("Update data!")
            ...
            elif "rate" in self.args.fill_policy:#按比例更新数据
                ...
                if epoch_i+1 > update_after:
                    if epoch_i == update_after:
                        ...
                        # 按比例混合原始数据和预测数据
                    data = data * (1 - update_rate) + data_pred * update_rate#逐步引入预测数据
            else:
                # no data update
                pass
            #在特定的训练轮次中切换到全监督学习模式，确保所有数据点都被考虑在内
            ...
            if hasattr(self.args, "data_pred"):
                if hasattr(self.args, "sample_period"):
                    ...
                    sample_period = 1
                ## 
                batch_gen = batch_generater(data, observ_mask, #生成一批次数据
                ...
                for x, y, t, mask in batch_gen:
                    ...
                    y_pred, loss = self.latent_data_pred(x, y, mask)#预测数据和损失值
                    data_pred[t] = (y_pred*(1-mask) + y*mask).clone().detach()[:,:,0]#更新预测数据，在掩码为0的地方
            ...
            if hasattr(self.args, "graph_discov"):
                ...
                for x, y, t, mask in batch_gen:#遍历 batch_gen 中的每个批次，x 是输入数据，y 是目标数据，t 是时间步，mask 是观察掩码
            ...
            pbar.update(1)
            self.lambda_s *= self.lambda_gamma#更新稀疏性正则化系数
            ...
            time_coef_mat = self.graph.detach().cpu().numpy()#获取图的系数矩阵
            plot_roc = False
            if (epoch_i+1) % self.args.show_graph_every == 0:#每隔一段时间绘制一次图
...
def prepare_data(opt):
    ...
    #创建了一个与输入数据data形状相同的掩码（mask）数组
    mask = np.ones_like(data)#指定周期缺失、随机生成周期缺失
    if hasattr(opt.pre_sample, "period") or hasattr(opt.pre_sample, "random_period"):
        ...
        for i in range(N):
            period_i = period[i]
            mask[::period_i, i] += 1 #表示在第i列每隔3个时间步设置为
    ...
    sampled_data = data * mask #得到含有缺失值的数据
...
def main(opt: CUTSopt, device="cuda"):
    ...
    if true_cm is not None:
        ...
        log.log_figures(name="True Graph", figure=sub_cg, iters=0) # 绘制真实因果图
    
    if hasattr(opt, "cuts"):
        cuts = CUTS(opt.cuts, log, device=device)
        cuts.train(data, mask, original_data, true_cm) # 训练CUTS模型
